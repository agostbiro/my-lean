...
/-- The `q0 --110--> q1` transition, used to show the proof generated by `decide`. -/
abbrev StepEndsWithCarry : Prop :=
  dfaStep (.carry false) (true, true, false) = .carry true

example : StepEndsWithCarry :=
  (of_decide_eq_true
    (Eq.refl true : decide StepEndsWithCarry = true))

-- The proof generated by decide with the proposition passed explicitly.
example :
    dfaStep (.carry false) (true, true, false) = .carry true := 
  of_decide_eq_true
    (p := dfaStep (.carry false) (true, true, false) = .carry true)
    (Eq.refl true)
